...
public class Board {
    ...
    List<ConcreteCoordinate> getCoordinates(Color color) {
        assert !color.isNull();

        List<ConcreteCoordinate> coordinates = new ArrayList<>();
        for (int i = 0; i < ROWS; i++) {
            for (int j = 0; j < COLUMNS; j++) {
                if (this.getColor(new ConcreteCoordinate(i, j)) == color) {
                    coordinates.add(new ConcreteCoordinate(i, j));
                }
            }
        }
        return coordinates;
    ...
    boolean isAvailableMove(int column) {
